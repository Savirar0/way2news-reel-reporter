  ...
  return (
    <section className="py-12 md:py-20 bg-brand-light border-y border-brand-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl md:text-3xl font-bold text-brand-dark text-center mb-10 md:mb-12">{content.sections.whoCanJoin}</h2>
          ...
          {content.personas.map((p, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col items-center gap-4 group cursor-default">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-white rounded-2xl flex items-center justify-center transition-transform group-hover:-translate-y-2 shadow-sm border border-brand-border group-hover:border-brand-yellow">
                  {/* Added fill-current to make icons solid */}
                  <Icon 
                    size={32} 
                    strokeWidth={1.5}
                ...
                <span className="font-semibold text-brand-dark text-sm tracking-wide text-center">{p.label}</span>
              </div>
            );
          })}
        </div>
